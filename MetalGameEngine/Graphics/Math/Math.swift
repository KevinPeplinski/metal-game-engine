...
extension matrix_float4x4 {
    mutating func translate(direction: SIMD3<Float>) -> Void {
    ...
    mutating func scale(axis: SIMD3<Float>) -> Void {
        var result = matrix_identity_float4x4
    ...
    mutating func rotate(angle: Float, axis: SIMD3<Float>) -> Void {
        var result = matrix_identity_float4x4
    ...
    static func perspective(fov: Float, aspectRatio: Float, near: Float, far: Float) -> matrix_float4x4 {
        var result = matrix_identity_float4x4

        let fovRadian = fov.radian

        let t: Float = tan(fovRadian / 2)
    ...
    static func orthographic(aspectRatio: Float, near: Float, far: Float) -> matrix_float4x4 {
        var result = matrix_identity_float4x4

        let t: Float = 0
        let b: Float = 1 * aspectRatio
        let l: Float = 0
        let r: Float = 1
        
        let x: Float = 2 / (r - l)
        let y: Float = 2 / (t - b)
        let z: Float = -1 / (far - near)
        let w1: Float = -((r + l) / (r - l))
        let w2: Float = -((t + b) / (t - b))
        let w3: Float = -(near / (far - near))

        result.columns = (
            SIMD4<Float>(x,  0,  0,   0),
            SIMD4<Float>(0,  y,  0,   0),
            SIMD4<Float>(0,  0,  z,   0),
            SIMD4<Float>(w1,  w2,  w3,   1)
        )
